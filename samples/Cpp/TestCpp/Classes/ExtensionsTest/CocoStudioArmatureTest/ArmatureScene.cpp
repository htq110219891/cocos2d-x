...
CCLayer *CreateLayer(int index)
{
    ...
    case TEST_ARMATURE_NESTING_2:
        ...
        break;
    case  TEST_ADD_REMOVE_BONE:
        pLayer = new TestAddRemoveBone();
    default:
        break;
    }
...
void TestAddRemoveBone::onEnter()
{
    ArmatureTestLayer::onEnter();
    ...
    bone = armature->getBone("Layer132");
    bone->retain();

    setTouchEnabled(true);
}
void TestAddRemoveBone::onExit()
{
    ArmatureTestLayer::onExit();
...
std::string TestAddRemoveBone::title()
{
    return "Test remove and add bone.";
}
std::string TestAddRemoveBone::subtitle()
{
    return "Touch the screen.";
}
bool TestAddRemoveBone::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
    if (bone->getArmature())
    {
    ...
    {
        armature->addBone(bone, "Layer122");
    }

    return false;
}
void TestAddRemoveBone::registerWithTouchDispatcher()
{
    CCDirector::sharedDirector()->getTouchDispatcher()->addTargetedDelegate(this, kCCMenuHandlerPriority + 1, true);
}
